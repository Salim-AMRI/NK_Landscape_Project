...
import cma  # Ajout de l'import CMA-ES

from neural_net import Net
from env_nk_landscape import EnvNKlandscape


def get_average_score_strategy(type_strategy, N, K, weights, network, path, nb_intances, nb_restarts):
    # Cloner le réseau neuronal pour éviter d'écraser les poids originaux
    ...
    return average_score/(nb_intances*nb_restarts)

# Fonction d'évaluation pour CMA-ES
def evaluate_weights_NN(N, K, solution, network, path, nb_intances, nb_restarts):
    ...
    average_score = get_average_score_strategy("NN", N, K, weights, network, path, nb_intances, nb_restarts)
...
print("eval hillclimber")
average_score_hillclimber = get_average_score_strategy("hillClimber", N, K, None, None, valid_path, nb_intances, nb_restarts)

print("average_score_hillclimber validation")
...
for generation in range(max_generations):
    ...
    training_scores = Parallel(n_jobs=nb_jobs)(delayed(evaluate_weights_NN)(N, K, solution, list_nnet[idx], train_path, nb_intances, nb_restarts)  for idx, solution in enumerate(solutions))

    for idx, reward in enumerate(training_scores):
        if reward > best_reward:
            best_reward = reward
            best_solution = solutions[idx]
            ### Tester best_solution sur un ensemble de validation, afficher le score et le comparer au score moyen du hillclimber sur ce même panier d'instance
            validation_score = evaluate_weights_NN(N, K, best_solution, list_nnet[0], valid_path, nb_intances,
                                                   nb_restarts)
            print("validation_score : " + str(validation_score))


    es.tell(solutions, -np.array(training_scores))  # Mettez à jour la distribution CMA-ES
