...
for instance_file in instance_files:
    try:
        env = EnvNKlandscape(instance_file)  # Initialisation de l'environnement NK depuis le fichier
    except Exception as e:
        print(f"Erreur lors de l'initialisation de l'environnement pour {instance_file}: {str(e)}")
        continue  # Passe à l'instance suivante en cas d'erreur

    layers_size = [N, 4 * N, 2 * N, N]
    nnet = Net(layers_size)
    params_net = list(nnet.parameters())

    x0 = np.random.rand(N)  # Solution initiale aléatoire dans [0, 1]
    initial_sigma = 0.1  # Écart-type initial

    es = cma.CMAEvolutionStrategy(x0, initial_sigma)

    best_score = 0
    pbar = tqdm(range(nb_generations))

    for generation in pbar:
        # Obtenir des solutions de CMA-ES
        solutions = es.ask()

        # Évaluation des solutions
        deltaFitness = Parallel(n_jobs=nb_jobs)(delayed(objective_function)(solution) for solution in solutions)

        es.tell(solutions, deltaFitness)  # Informer CMA-ES des récompenses

        # Meilleur score dans la génération actuelle
        current_score = np.max(deltaFitness)

        if current_score > best_score:
            best_score = current_score

        pbar.set_postfix(current_score=current_score, best_score=best_score)
