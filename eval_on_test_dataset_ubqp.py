...
list_strategy_name = ["hillClimberJump","hillClimberFirstImprovementJump", "oneLambdaDeterministic", "strategyNN", "StrategyNNFitness_and_current", "strategyNNRanked_v2","strategyNNRanked_v2_zScore"]
            ...
            for N in [32,64,128,256]:
                for K in [0,1,2,3,4,5]:
                    ...
                    if (type_strategy == "strategyNN"):
                        ...
                        list_strategy = [OneLambdaDeterministic(N, int(N/2)) for idx_run in range(nb_instances * nb_restarts)]
                    ...
                    if("NN" in type_strategy):
                        ...
                        list_scores = Parallel(n_jobs=nb_jobs)(delayed(get_Score_trajectory)(type_problem, list_strategy[idx_run], N, K, path, nb_instances, idx_run, alpha=None, withLogs=False, starting_point = list_starting_points[idx_run]) for idx_run in range(nb_instances * nb_restarts))
                    else:
                    ...
                    if(type_problem == "NK"):
                        ...
                        print(type_strategy + " : " + str(np.mean(list_scores)))
                ...
                # Enregistrer les résultats dans un fichier texte
                    ...
                    if (type_problem == "NK"):
                        mean_score = np.mean(list_scores)/ N
                    else:
                        mean_score = np.mean(list_scores)
                    result_file.write(f"  {type_strategy}: {mean_score}\n")
