...
def InvariantModule(module_class, N, remix, rescale):
    ...
    class _InvariantModuleClass(th.nn.Module):
        ...
        def forward(self, *inputs):
            if remix:
                ...
                if(rescale):
                    return N * self._ind_module(*inputs) / (N + 1) + self._mean_module(
                        *red_inputs
                    ) / (N + 1)
...
class InvariantNNet(nn.Module):
    def __init__(self,  N, remix, rescale, layers_size):
        super().__init__()
        layers = []
        ...
        InvariantLinear = InvariantModule(th.nn.Linear, self.N, self.remix, rescale)
